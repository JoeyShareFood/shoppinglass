...
/*********************** rendimentos de hoje */
$sql =  "select sum(origin) as income_pay from `order` where state='unpay' and DATE(`datapedido`)  = '$daytime'";
$rs = mysql_query($sql);
$row = mysql_fetch_object($rs); 
...
/*********************** rendimentos do mes */
$sql =  "SELECT sum( origin ) AS income_pay FROM `order` WHERE state = 'unpay' AND MONTH( `datapedido` ) = MONTH( NOW( ) ) AND YEAR( `datapedido` ) = YEAR( NOW( ) ) ";
$rs = mysql_query($sql);
$row = mysql_fetch_object($rs); 
$rendimento_mes = $row->income_pay;
if(!$rendimento_mes) $rendimento_mes = 0; 
/*********************** rendimentos do ano */
$sql =  "select sum(origin) as income_pay from `order` where state='unpay' and YEAR(`datapedido`) = YEAR(NOW())";
$rs = mysql_query($sql);
$row = mysql_fetch_object($rs); 
$rendimento_ano = $row->income_pay; 
if(!$rendimento_ano) $rendimento_ano = 0; 

/*********************** rendimentos de hoje pagos */
$sql =  "select sum(origin) as income_pay from `order` where state='pay' and DATE(`datapedido`)  = '$daytime'";
$rs = mysql_query($sql);
$row = mysql_fetch_object($rs); 
$rendimento_hoje_pago = $row->income_pay; 
if(!$rendimento_hoje_pago) $rendimento_hoje_pago = 0; 

/*********************** rendimentos do mes pagos*/
...
$rs = mysql_query($sql);
$row = mysql_fetch_object($rs); 
$rendimento_mes_pago = $row->income_pay;
if(!$rendimento_mes_pago) $rendimento_mes_pago = 0; 
/*********************** rendimentos do ano pagos*/
$sql =  "select sum(origin) as income_pay from `order` where state='pay' and YEAR(`datapedido`) = YEAR(NOW())";
$rs = mysql_query($sql);
$row = mysql_fetch_object($rs); 
$rendimento_ano_pago = $row->income_pay; 
if(!$rendimento_ano_pago) $rendimento_ano_pago = 0; 


/*********************** rendimentos de hoje pagos */
$sql =  "select sum(comissaosite) as comissao from `order` where state='pay' and DATE(`datapedido`)  = '$daytime'";
$rs = mysql_query($sql);
$row = mysql_fetch_object($rs); 
$comissoes_hoje = $row->comissao; 
if(!$comissoes_hoje) $comissoes_hoje = 0; 

/*********************** rendimentos do mes pagos*/
$sql =  "SELECT sum( comissaosite ) AS comissao FROM `order` WHERE state = 'pay' AND MONTH( `datapedido` ) = MONTH( NOW( ) ) AND YEAR( `datapedido` ) = YEAR( NOW( ) ) ";
$rs = mysql_query($sql);
$row = mysql_fetch_object($rs); 
$comissoes_mes = $row->comissao;
if(!$comissoes_mes) $comissoes_mes = 0; 
/*********************** rendimentos do ano pagos*/
$sql =  "select sum(comissaosite) as comissao from `order` where state='pay' and YEAR(`datapedido`) = YEAR(NOW())";
$rs = mysql_query($sql);
$row = mysql_fetch_object($rs); 
$comissoes_ano = $row->comissao; 
if(!$comissoes_ano) $comissoes_ano = 0; 
