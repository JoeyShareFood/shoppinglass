...
if($tipo=="header"){
	...
	$filefisico = WWW_ROOT."/skin/padrao/header/".$file;
	if(file_exists( $filefisico)){
		if(!unlink($filefisico)){
			 echo "Não foi possível apagar fisicamente a foto";	
		}
	}else{
		echo "Este arquivo não existe mais";
	}
	exit;
}
if($tipo=="background"){
	
	$filefisico = WWW_ROOT."/skin/padrao/background/".$file;
	if(file_exists( $filefisico)){
		if(!unlink($filefisico)){
			 echo "Não foi possível apagar fisicamente a foto";	
		}
