...
include(dirname(dirname(dirname(__FILE__))) ."/templates/template_new.class.php");
...
function mail_sign_cadastro($user) {
	...
	global $INI, $PATHSKIN, $ROOTPATH;
	if ( empty($user) ) return true;
	$to = $user['email'];

	$template = new Template_new('cadastro.html');
	$template->set("baseurl", $ROOTPATH);
	$template->set("nomesite", $INI['system']['sitename']);
	$template->set("skinurl", $PATHSKIN);
	$template->set("skinurlfooter", $PATHSKIN . "/images/rodape_email.jpg");
	$template->set("skinurltop", $PATHSKIN . "/images/topo_email.jpg");
	$template->set("nome", strtoupper($user['realname']));
	$template->set("usuario", $user['username']);
	$template->set("senha", $user['senha']);
	$template->set("ano", date("Y"));
	$template->set("facebook", $INI['other']['facebook']);
	$template->set("instagram", $INI['other']['orkut']);

	$assunto = 'Obrigado por se cadastrar no '.$INI['system']['sitename'];
	enviar($to,$assunto,$template->output());	  
}

function mail_sign_cadastro_partner($user) {
 
	global $INI;
	if ( empty($user) ) return true;
	$to = $user['email'];

	$template = new Template_new('cadastro_partner.html');
	$template->set("baseurl", $ROOTPATH);
	$template->set("nomesite", $INI['system']['sitename']);
	$template->set("skinurl", $PATHSKIN);
	$template->set("nome", $user['nome_negocio']);
	$template->set("usuario", $user['username']);
	$template->set("senha", $user['senha']);
...
function mail_repass($user) {
	...
	$assunto = 'Envio de senha - '.$INI['system']['sitename'];

	$template = new Template_new('repass.html');
	$template->set("baseurl", $ROOTPATH);
	$template->set("nomesite", $INI['system']['sitename']);
	$template->set("skinurl", $PATHSKIN);
	$template->set("skinurlfooter", $PATHSKIN . "/images/rodape_email.jpg");
	$template->set("skinurltop", $PATHSKIN . "/images/topo_email.jpg");
	$template->set("nome", strtoupper(utf8_decode($user['realname'])));
	$template->set("usuario", $user['email']);
	$template->set("senha", $user['senha']);
	$template->set("ano", date("Y"));
	$template->set("facebook", $INI['other']['facebook']);
	$template->set("instagram", $INI['other']['orkut']);
