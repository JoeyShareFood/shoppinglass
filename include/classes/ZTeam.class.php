...
class ZTeam
{
	...
	static public function BuyOne2($order,$off) {

		global $INI;
 
		Util::log($order['id']. " - Alteracao de status do pedido para pago a partir da administração ou compras com credito. (nao e pagseguro)");
		...
		ZFlow::CreateFromOrder($order);
		/* order : send coupon ? */
		ZCoupon::CheckOrder($order,false,$off);
		/* order : invite buy */
		ZInvite::CheckInvite($order);
		/* credit */
		ZCredit::Buy($order['user_id'], $order);
	...
	static public function BuyOne3($order) { // pagamento com creditos
		...
		$team = Table::Fetch('team', $order['team_id']);
		 ...
		 if($team['now_number'] >= $team['min_number']){  //<!--  A oferta esta ativa  --> 
			...
			$sql = "select a.id,a.secret,a.order_id,a.nome,b.username,b.email,c.title,c.homepage,c.location,c.address,c.chavesms from coupon a,user b, partner c where a.partner_id = c.id and a.envioucupom is null and a.user_id = b.id and a.team_id = ".$order['team_id'];
		    $rs = mysql_query($sql);
