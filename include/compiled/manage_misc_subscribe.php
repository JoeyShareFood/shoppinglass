						...
						<div class="top-heading group"> <div class="left_float"><h4>Usuários</h4></div> 
							<div style="padding: 10px;">	
								<ul class="filter" style="top:47px;">
										<li style="margin-top:-14px;"> <button  onclick="javascript:location.href='<?=$ROOTPATH?>/vipmin/market/downemail.php'"  id="run-button" class="input-btn" type="button"><div name="spinner-top" id="spinner-top" style="width: 83px; display: block;"><img name="imgrec" id="imgrec" src="<?=$ROOTPATH?>/media/css/i/lendo.gif" style="display: none;"></div><div id="spinner-text"  >Exportar E-mails</div></button></li>
								       <li><form action="/vipmin/misc/subscribe.php" method="get"> Email:<input type="text" name="like" value="<?php echo htmlspecialchars($like); ?>" class="h-input" />&nbsp;<input type="submit" value="Procurar" class="formbutton"  style="padding:1px 6px;"/><form></li>
 								</ul>
							</div>	
						</div> 
				 ...
				 <div class="paginacaotop" style="width:50%"><?php echo $pagestring; ?></div>
					...
					<div class="sect">
						...
						<tr <?php echo $index%2?'':'class="alt"'; ?> id="team-list-id-<?php echo $one['id']; ?>">
							...
							<td class="op" nowrap><a ask="Deletar ou não?" href="/ajax/manage.php?action=subscriberemove&id=<?php echo $one['id']; ?>" class="ajaxlink" style="">deletar</a></td>
						</tr>
						<?php }}?>
...
<script>
  function msg(){
		jQuery.colorbox({html:"<div class='msgsoft'><img src='<?=$PATHSKIN?>/images/ajax-loader2.gif'> Deletando este email...</div>"});
	}  
	</script>
