				...
				<div class="option_box">
					...
					<div class="sect" style="clear:both;">
						...
						<tr <?php echo $index%2?'class="normal"':'class="alt"'; ?> id="team-list-id-<?php echo $one['id']; ?>">
							...
							<td><?php if($one['status']=="1") { echo "Sim";} else { echo "<span style='color:orange'>Não</span>";} ?></td> 
							<td><?php echo date('d/m/Y H:i:s', strtotime($one['data_criacao'])); ?></td> 
							<td><?php  if($one['datamodificacao'] != ""){ echo date('d/m/Y H:i:s', strtotime($one['datamodificacao']));} else { echo "-"; } ?></td> 
							<td class="op" nowrap>
