...
define('STATUS_APROVADO', 'Aprovada'); 

define('STATUS_COMPLETO', 'Concluída'); 

define('STATUS_DEVOLVIDO', 'Devolvida'); 
...
/*++++++++++++++++++++++++++++++++++++++++++++++++ CRIAÇÃO DO ARRAY DOS DADOS DO PAGAMENTO DO GATEWAY*/
 ...
 $dados_pagamento = array(
    ...
    "status_transacao" => $_POST['status'], 

    "tipo_pagamento" => $_POST['tipo_pagamento'],  

    "data_pagamento" => $_POST['data_transacao'],  

    "valor_unitario" => $_POST['valor_original'],  

    "quantidade_comprada" => $_POST['qtde_produtos'],  
...
mail("danieltisa@vipcomsistemas.com.br","retorno de dados ".$RetornoPagamento->gateway,$RetornoPagamento->gravar_request()); 	 
...
if ($RetornoPagamento->idPedido and $_POST) {
	...
	Util::log($_POST['id_pedido']." - Descricao status: ".utf8_decode($_POST['status'])."-".$_POST['cod_status']); 
...
header("Location: ".$ROOTPATH."/index.php?page=retornopagamento");	

?>

<meta http-equiv="refresh" content="0; url=<?=$ROOTPATH?>?page=retornopagamento">