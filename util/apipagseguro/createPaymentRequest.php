...
class createPaymentRequest {
	...
	public static function main () {
		...
		global $INI,$title,$valor_original,$idpedido,$ROOTPATH,$login_user,$nomes,$valorfrete,$valorcupomdesconto ;
		...
		//$paymentRequest->addItem('0001', 'Notebook prata', 2,430.00);
		 ...
		 /*
		 $data = Array(  
			'id' => '0001', // identificador  
			'description' => 'Notebook Prata', // descrição  
			'quantity' => 1, // quantidade  
			'amount' => 2430.00, // valor unitário  
			'weight' => 1000, // peso em gramas  
			'shippingCost' => 17.35 // custos de frete  
		);  
		*/

		// Add another item for this payment request		$valor_original = str_replace(".","",$valor_original);	
		//$valor_original =  str_replace(",",".",$valor_original);echo "======vip: ".$valor_original;
		$valorapi = str_replace(",","-",$valor_original);
		$valorapi = str_replace(".","",$valorapi);
		$valorapi = str_replace("-",".",$valorapi);
		if($valorfrete !=""){
			$paymentRequest->addItem($idpedido, utf8_decode($title), 1, $valorapi,1,$valorfrete);
		 }
		 else{
			$paymentRequest->addItem($idpedido, utf8_decode($title), 1, $valorapi);
		 }
		
		// Sets a reference code for this payment request, it is useful to identify this payment in future notifications.
		$paymentRequest->setReference($idpedido);
		
		$paymentRequest->setExtraAmount($valorcupomdesconto);
