 <?php 
	...
	$user = Table::Fetch('user', $offer['partner_id']);
...
<div class="titulo" style="background:#20547E; box-shadow: 2px 2px 4px 0 #888888;font-family: helvetica;font-size: 19px; height: 28px; padding: 0px 12px 0;color:#FFF;margin-bottom:16px;">Produto cadastrado</div> 

 

 <p>Clique <a href="<?php echo $ROOTPATH; ?>/vipmin/order/index.php">aqui</a> para moderar este anúncio.</p>
 <b>ID da oferta: </b><?=$offer['id']?>
 <BR><b>Título: </b><?=$offer['title']?>
 <?php if(!(empty($offer['codreferencia']))) { ?><BR><b>Código: </b><?=$offer['codreferencia']?><?php } ?>
 <BR><b>Valor: </b>R$<?=number_format($offer['team_price'], 2, ",", ".")?><BR><BR>
 <b>Dados do usuário</b><BR>
 <BR><b>Nome do usuário: </b><?=$user['realname']?>
