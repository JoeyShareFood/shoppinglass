										...
										<tbody>
											<tr style="font-size: 14px;   color: #303030; padding: 2px 0px; margin: 0px; font-family: 'Verdana';">
												<td style="padding: 4px;border:1px solid #DCDCDC;" valign="top" width="57%">
													...
													<div class="titulo" style="background:#20547E; box-shadow: 2px 2px 4px 0 #888888;font-family: helvetica;font-size: 19px; height: 28px; padding: 0px 12px 0;color:#FFF;margin-bottom:16px;">Você recebeu um pagamento</div> 
														<p>O pagamento do pedido #<?php echo $id; ?> foi liberado pelo site com sucesso!</p>
														...
														<?php if(!(empty($team['team_price']))) { ?><p>Valor da oferta: R$ <?php echo moneyit3($team['team_price']); ?></p><?php } ?>
														<?php if(!(empty($order['quantity']))) { ?><p>Quantidade comprada: <?php echo $order['quantity']; ?></p><?php } ?>
														<?php if(!(empty($order['comissaosite']))) { ?><p>Comissão do site: R$ <?php echo moneyit3($order['comissaosite']); ?></p><?php } ?>
														<?php if(!(empty($order['valorvendedor']))) { ?><p>Total recebido: R$ <?php echo moneyit3($order['valorvendedor']); ?></p><?php } ?>
														<p><b><?=$nomesite ?></b></p> </td>
